...
@user_passes_test(_superuser_check, login_url="admin_login")
def admin_media_health_view(request):
    ...
    try:
        ...
        result["checks"]["path_exists"] = os.path.exists(media_root)
        result["checks"]["is_directory"] = os.path.isdir(media_root)

        test_file_name = f".render_media_health_{uuid4().hex}.txt"
        test_file_path = os.path.join(media_root, test_file_name)
        payload = "render-media-health-ok"
        ...
        result["checks"]["read_test"] = read_back == payload

        os.remove(test_file_path)
        result["checks"]["cleanup_test"] = not os.path.exists(test_file_path)
...
def admin_blog_list_view(request):
    ...
    try:
        ...
        blogs_page = paginator.page(paginator.num_pages)
    serialized_blogs = BlogAdminListSerializer(
        blogs_page.object_list, many=True, context={"request": request}
    ).data
...
def admin_blog_detail_view(request, pk):
    ...
    return render(
        request, "admin_blog_detail.html", {"blog_data": blog_data, "blog": blog}
    )
...
def admin_category_list_view(request):
    ...
    return render(
        ...
        {"categories": categories_page, "filter_status": filter_status},
    )
...
def admin_category_add_view(request):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            form_errors["name"] = (
                "Category name must contain letters only and single spaces between words."
            )
...
def admin_category_edit_view(request, pk):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            BlogCategory.objects.filter(name__iexact=name)
            ...
            if not category.slug or name_changed:
                category.slug = _generate_unique_slug_for(
                    BlogCategory, name, instance_id=category.pk
                )
            category.save()
...
def admin_tag_list_view(request):
    ...
    return render(
        request, "admin_tags.html", {"tags": tags_page, "filter_status": filter_status}
    )
...
def admin_tag_add_view(request):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            form_errors["name"] = (
                "Tag name must contain letters only and single spaces between words."
            )
...
def admin_blog_add_view(request):
    ...
    if request.method == "POST":
        ...
        if _has_leading_or_trailing_space(title_raw):
            ...
            form_errors["title"] = (
                "Title must not contain multiple spaces between words."
            )
        ...
        if _has_leading_or_trailing_space(description_raw):
            form_errors["description"] = (
                "Full description must not have leading or trailing spaces."
            )
        elif _has_multiple_spaces(description_raw):
            form_errors["description"] = (
                "Full description must not contain multiple spaces between words."
            )
        if _has_leading_or_trailing_space(author_name_raw):
            form_errors["author_name"] = (
                "Author name must not have leading or trailing spaces."
            )
        elif _has_multiple_spaces(author_name_raw):
            form_errors["author_name"] = (
                "Author name must not contain multiple spaces between words."
            )
        elif not _is_valid_name(author_name):
            form_errors["author_name"] = (
                "Author name must contain letters only and single spaces between words."
            )
        if form_errors:
            return render(
                request,
