...
def ckeditor_upload_view(request):
    ...
    if uploaded_file.content_type not in allowed_types:
        return JsonResponse(
            {"error": "Only JPG, PNG, and GIF files are allowed"}, status=400
        )
...
class BlogViewSet(
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = BlogSerializer(
            blog, data=request.data, partial=partial, context={"request": request}
        )
        if serializer.is_valid():
            updated_blog = serializer.save()
            response_serializer = BlogDetailSerializer(
                updated_blog, context={"request": request}
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return Response(
            {"detail": "Blog soft-deleted successfully."},
        ...
        if blog_id:
            try:
                ...
                Blog.objects.filter(id=blog_id).update(
                    view_count=Blog.objects.get(id=blog_id).view_count + 1
                )
...
class BlogFrontendViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        blogs = (
            blogs.prefetch_related("categories", "tags", "gallery")
            .order_by("-created_at")
            .distinct()
        )
        categories = BlogCategory.objects.filter(is_active=True).order_by("name")
...
class BlogAdminViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        serializer = BlogAdminListSerializer(
            queryset, many=True, context={"request": request}
        )
...
def download_license_csv(request):
    ...
    queryset = LicenseModel.objects.select_related("generatedBy", "linkUser").all()
...
class BlogSitemap(Sitemap):
    ...
    def items(self):
        return Blog.objects.filter(is_active=True).exclude(slug="")
